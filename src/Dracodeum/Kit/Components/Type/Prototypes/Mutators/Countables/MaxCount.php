...
/**
 * This prototype restricts a given countable value to a maximum count.
 * 
 * @property-write int $count [writeonce] [transient]  
 * The count to restrict to.
 */
class MaxCount extends Prototype implements IExplanationProducer
{
	...
	{
		return Text::build()
			->setString("Only up to {{count}} value is allowed.")
			->setPluralString("Only up to {{count}} values are allowed.")
